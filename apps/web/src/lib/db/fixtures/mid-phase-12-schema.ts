/**
 * THE SCHEMA AS IT STOOD PART-WAY THROUGH PHASE 12, FROZEN.
 *
 * Generated once from `schema.ts` at the point where the provisional Discovery
 * Board had landed and the final sprint had not, with the explanatory comments
 * stripped — and then never touched again. Like its pre-Phase-12 sibling, it is
 * a fixture rather than a copy of anything live: update it to match the current
 * schema and the migration test starts migrating the present to the present and
 * passes forever while proving nothing.
 *
 * What it has that `PRE_PHASE_12_SCHEMA` does not: `decision_sessions`,
 * `provisional_boards`, `provisional_selections`, `board_reconciliations`,
 * `destination_images`, `stage_observations`.
 *
 * What it still lacks, and what the final sprint therefore has to add without
 * disturbing anything: `interpretation_cache`, `weather_snapshots`,
 * `board_reconciliations.acknowledged_json`, `compilation_jobs.operational_json`.
 */
export const MID_PHASE_12_SCHEMA = `
